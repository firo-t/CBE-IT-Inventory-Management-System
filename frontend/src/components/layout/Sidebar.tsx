...
export default function Sidebar() {
  ...
  useEffect(()=>{ const s=getSession(); if(s) setRole(s.role); },[]);
  return <aside className="fixed left-0 top-0 z-30 flex h-screen w-64 flex-col bg-[#9b1749] text-white">
    ...
    <button onClick={()=>{clearSession();location.href="/login"}} className="m-3 flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm text-white/80 hover:bg-white/10"><LogOut size={18}/>Sign out</button>
  </aside>;
}
